...
// createNfSaida, getAllNfs - permanecem como estavam
...
        
        const retornosParciais = await NotaFiscal.find({ nfNumero, tipo: 'Entrada' }).sort({ dataEntrada: 1 }).lean();

        const radiosComDetalhes = await Promise.all(nfSaida.radios.map(async (numeroSerie) => {
            // AQUI ESTÁ A CORREÇÃO: Adicionado 'numeroSerie' ao .select()
