...
// Define a URL base da sua API em um só lugar
...
    if(tabelaRadiosBody) tabelaRadiosBody.innerHTML = '<tr><td colspan="8" class="text-center">Carregando rádios...</td></tr>';

    try {
        const token = localStorage.getItem('token');
        // ALTERADO: Usando a URL base e o caminho correto
...
            showAlert('Erro ao Carregar Estoque', `Falha ao carregar dados: ${errorData.message || res.statusText}`, 'danger');
...
        todosRadios = Array.isArray(data) ? data : [];
        popularSubFiltro();
        aplicarFiltro();
...
        tbody.innerHTML = '<tr><td colspan="8" class="text-center">Nenhum rádio no estoque de locação.</td></tr>';
        return;
    }
...
        const correspondeSubfiltro = !tipoFiltro || !subFiltroValorSelecionado || (r[tipoFiltro] === subFiltroValorSelecionado);
...
        const tr = document.createElement('tr');
        // A lógica de renderização da tabela permanece a mesma...
        tr.innerHTML = `
            <td>${r.modelo || 'N/A'}</td>
            ...
            <td>...</td>
        `;
        // Recria a lógica do status e do botão de detalhes para garantir que os eventos sejam adicionados
...
    const patrimonioAtualExibido = document.getElementById('editPatrimonioAtual').value;

    if (!novoPatrimonio) {
        showAlert('Campo Obrigatório', 'O novo patrimônio não pode ser vazio.', 'warning');
        return;
    }
    if (novoPatrimonio === patrimonioAtualExibido) {
        showAlert('Sem Alterações', 'O novo patrimônio é o mesmo que o atual.', 'info');
...
    try {
        const token = localStorage.getItem('token');
        // ALTERADO: Usando a URL base e o caminho correto com '/serial/'
...
            },
            body: JSON.stringify({ novoPatrimonio }) // Alterado para enviar 'novoPatrimonio'
        });
...
    const token = localStorage.getItem('token');
    if (!token) {
        showAlert('Erro', 'Usuário não autenticado.', 'danger');
        return;
    }

    try {
        // ALTERADO: Usando a URL base e o caminho correto para notas fiscais
